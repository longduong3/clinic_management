...
namespace HospitalManagement.Areas.Admin.Controllers
{
    ...
    {
        private readonly string KeyElement = "Phiếu khám";
        ...
        public JsonResult Del(Guid Id)
        {
            try
            {
                using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
                {
                    var elm = workScope.DetailRecords.FirstOrDefault(x => x.Id == Id && !x.IsMainRecord);
                    var user = GetCurrentUser();
                    if (user.Role == RoleKey.Doctor && elm.DoctorId != user.DoctorId)
                        {
                            ...
                            {
                                status = false,
                                mess = "Bạn không có quyền thực hiện thao tác này."
                            });
                        }
                    if (elm != null)
                    {
                        elm.IsMainRecord = true;
                        workScope.DetailRecords.Put(elm, elm.Id);
                        workScope.Complete();
                    ...
                    {
                        return Json(new { status = false, mess = "Bản ghi " + KeyElement + " không tồn tại." });
                    }
                }
            }
            catch
            {
                return Json(new { status = false, mess = "Thao tác thất bại." });
            }
        }
    }
}