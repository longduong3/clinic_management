...
namespace HospitalManagement.Areas.Admin.Controllers
{
    public class workController : BaseController
    {
        ...
        [HttpGet]
        public ActionResult DoctorScheduleDetail()
        {
            var userIdentity = GetCurrentUser();

            using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
            {
                if (userIdentity.Role == RoleKey.Doctor)
                {
                    var patientOfDoctors =
                        workScope.PatientDoctors.Query(x => x.DoctorId == userIdentity.DoctorId) ?? new List<PatientDoctor>();

                    var patientOfDoctorIds = patientOfDoctors.Select(x => x.PatientId);

                }
                var doctorSchedule = workScope.DoctorSchedules.Include(x => x.Patient).ToList().Where(x => x.DoctorId == userIdentity.DoctorId);
                var data = doctorSchedule.GroupBy(x => new { StartDate = x.ScheduleBook.Date }).ToList().SelectMany(x => x.Select(z => new DoctorScheduleViewModel()
                {
                    StartDate = x.Key.StartDate.ToString("yyyy-MM-dd"),
                    FrameTime = z.FrameTime,
                    PatientName = z.Patient?.FullName,
                    ScheduleBook = z.ScheduleBook,
                    Status = z.Status,
                }).ToList());
